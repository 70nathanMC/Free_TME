...
# Text characters with a slight neon glow
image digit_zero = Text("0", size=18, color="#32cd32", outlines=[(1, "#32cd3280", 0, 0)])
image digit_one = Text("1", size=18, color="#32cd32", outlines=[(1, "#32cd3280", 0, 0)])

# One shoots 0s, the other shoots 1s
image hack_zeros = SnowBlossom(
    "digit_zero",
    count=40,          
    border=0, 
    xspeed=(-20, 20),   
    yspeed=(-200, -400), # Shooting UPWARDS
    start=0,
    fast=True
)

image hack_ones = SnowBlossom(
    "digit_one",
    count=40,          
    border=0, 
    xspeed=(-20, 20),   
...
# Container
image hack_success_particles:
    contains:
        "hack_zeros"
    contains:
        "hack_ones"

image vent_dust = SnowBlossom(
    Transform("dust_mote.png", zoom=0.12),
    count=150,          
    border=50, 
    xspeed=(-5, 5),    
    yspeed=(-3, -8),   
...
image vent_dust_ending_01 = SnowBlossom(
    Transform("dust_mote.png", zoom=0.12), 
    count=200,         
    border=50, 
    xspeed=(-15, 15),  
...
image vent_dust_ending_02 = SnowBlossom(
    Transform("dust_mote.png", zoom=0.2),
    count=200,          
    border=50, 
    xspeed=(-15, 15),  
    yspeed=(-15, -25), 
...
image vent_dust_ending_03 = SnowBlossom(
    Transform("dust_mote.png", zoom=0.3), 
    count=200,        
    border=50, 
    xspeed=(-15, 15),   
    yspeed=(-15, -25), 
...
label start:
    ...
    menu:
        ...
        "Slip to Vent Section." if dev_powers == True:

            play music "audio/bgm/vent_bgm.ogg" fadeout 1.0 fadein 1.0
            play ambient "audio/misc/breathing.ogg"
            play environment "audio/bgm/vent_sfx.ogg"

            $ free_time_remaining = 100
            $ food = 100
            $ water = 100
    ...
    play music "audio/bgm/prologue_bgm.ogg" fadein 1.0
    ...
    s "Well, let's say that YOU did something HORRIBLY bad"
    ...
    s "All I can say is this. You chose to sell your body and mind to use in exchange for not rotting away in prison."
...
label tutorial_scene:
    ...
    play music "audio/bgm/tutorial_bgm.ogg" fadeout 1.0 fadein 1.0
...
label tutorial_hack_loop:
    ...
    b "For example, if your guess is 525, the answer could be anywhere from 515 to 524 (within 10 digits lower) or 526 to 535 (within 10 digits higher)."
...
label prologue:
    ...
    play music "audio/bgm/prologue_bgm.ogg" fadeout 1.0 fadein 1.0
    ...
    play music "audio/bgm/prison_bgm.ogg" fadeout 1.0 fadein 1.0
...
label before_work:
    ...
    play music "audio/bgm/main_menu_bgm.ogg" fadeout 1.0 fadein 1.0
...
label work_hours:
    ...
    play music "audio/bgm/hacking_bgm.ogg" fadeout 1.0 fadein 1.0
...
label hacking_loop:
    ...
    if player_guess < 100 or player_guess > 999:
        play sound "audio/ui/error_input.ogg"
        "ERROR. PLEASE INPUT A 3 DIGIT NUMBER ONLY."
        jump hacking_loop
        
    # 3. The Logic Check!
    if player_guess == correct_code:
        play sound "audio/ui/successful_input.ogg"
        # TRIGGER THE DIGITAL BURST!
        show hack_success_particles 
        
        "SYSTEM: DECRYPTION SUCCESSFUL."
        
        # Turn them off so they don't stay on the screen forever
        hide hack_success_particles 
        jump after_work_01
        ...
        "Spend TME" (cost=0):
            ...
            if spend_type == "food" or spend_type == "water":
                ...
                if input_number == 0:
                    ...
                    play sound "audio/ui/free_time_purchased.ogg"
...
label free_time:
    play music "audio/bgm/free_time_bgm.ogg" fadeout 1.0 fadein 1.0
    show screen status_hud
    scene room_overview
...
label end_free_time:
    ...
    play sound "audio/ui/free_time_done.ogg"
...
label vent_maze_loop:
    ...
    # The Choice Menu
    menu:
        "Left" (cost=5, vent=True):
            $ chosen_direction = "Left"
        "Middle" (cost=5, vent=True):
            $ chosen_direction = "Middle"
        "Right" (cost=5, vent=True):
            $ chosen_direction = "Right"
            
        "Get back to the start of vents" (cost=vent_progress * 5, vent=True) if vent_progress > 0:
        ...
        "Get back in the office" (cost=3, vent=True) if vent_progress == 0:
            stop ambient fadeout 1.0
            stop environment fadeout 1.0
            hide screen status_hud
            hide vent_dust
...
label vent_exit_found:
    ...
    show vent_dust_ending_01
    y "Wait... is that light?"
    
    scene light_tunnel_02
    show vent_dust_ending_02
    y "I..."
    
    scene light_tunnel_03
    show vent_dust_ending_03
    y "Yes! I made it!"
    ...
    window hide
    hide vent_dust_ending_03
    scene black_screen with fade
    centered "Congrats. You have escaped the room in [days_passed] days."
    centered "..."
    ...
    centered "Suddenly..."
    play sound "audio/misc/electrecution.ogg"
    show screen shock_timer
    centered "You were then suddenly electrocuted and you have passed out."
    stop sound fadeout 2.0
    centered "After a while..."
    centered "You then woke up"
    play music "audio/bgm/final_scene_bgm.ogg" fadein 2.0
    window auto
    y "..."
    think "I can't move or see."
    y "!!!"
    think "Seems that I am tied to a chair, and blindfolded."
    think "Where am I? What happened?"
    think "Oh yeah, I was about to escape the vents..."
    think "!"
    think "And then..."
    think "What I saw there was chilling..."
    scene ending_01 with fade
    play ambient "audio/misc/robot_noises.ogg"
    think "They were like humanoid robots that were seemingly waiting for me."
    think "Like they absolutely knew about my escape plan and that they knew everything that was happening."
    stop ambient
    scene black_screen with fade
    play sound "audio/misc/electrecution.ogg"
    show screen shock_timer
    think "Then... I suddenly got electrocuted and passed out."
    stop sound fadeout 2.0
    think "Damnit, what were those things? They feel like humanoid robots straight from sci-fi movies. So uncanny and terrifying."
    scene ending_01 with fade
    play ambient "audio/misc/robot_noises.ogg"
    think "What are they? Those models are way more advanced than the state of the art humanoid robots from what I can remember."
    think "Feels like they were truly conscious..."
    stop ambient
    scene black_screen with fade
    think "And have very high intelligence..."
    ...
    a "Like the cipher, the screwdriver, the vent being huge enough for you to crawl into, made sure to be clean enough as well too."
    y "..."
    ...
    y "Is it just to have fun seeing me suffer? Giving me hope just to take it away in the end?"
    ...
    a "Oops, you were not supposed to know that, but before you process that thought and get more angry and emotional, We will now proceed with the memory removal procedure."
...
label game_over:
    ...
    stop ambient fadeout 1.0
    stop environment fadeout 1.0
    play sound "audio/ui/game_over.ogg"
    pause 3.0  # Adjust this number to match the length of your game_over.ogg file!
...
screen room_exploration_ui():
    ...
    button:
        ...
        hover_sound "audio/ui/hover.ogg"
        activate_sound "audio/ui/vent_click.ogg"
    ...
    button:
        ...
        activate_sound "audio/ui/drawer_open.ogg"
    ...
    textbutton "> END FREE TIME // SLEEP":
        ...
        hover_sound "audio/ui/hover.ogg"
        activate_sound "audio/ui/confirm_action.ogg"
