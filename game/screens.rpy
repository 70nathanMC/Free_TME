...
init python:
    renpy.music.register_channel("terminal_track", mixer="sfx", loop=True)
    renpy.music.register_channel("ambient", mixer="sfx", loop=True)
    renpy.music.register_channel("environment", mixer="sfx", loop=True)
...
screen choice(items):
    ...
    vbox:
        for i in items:
            ...
            # 1. Look for our custom "vent" argument
            $ is_vent = i.kwargs.get("vent", False) if i.kwargs else False

            # 2. Automatically decide the sound based on the arguments!
            if is_vent:
                $ btn_sound = "audio/ui/vent_click.ogg"
            elif cost == 0 or cost is None:
                $ btn_sound = "audio/ui/confirm.ogg"
            else:
                $ btn_sound = "audio/ui/confirm_action.ogg"
            ...
            button at choice_zoom:
                ...
                hbox:
                    ...
                    if cost is not None:
                        # (Using the safer f-string here from the bug-fix pass!)
                        text ( f"-{cost} MIN" if cost > 0 else "FREE" ):
                            yalign 0.5
                            xalign 1.0
                            font gui.text_font
                            size 28 
...
style quick_button:
    ...
    hover_sound "audio/ui/hover.ogg"
    activate_sound "audio/ui/main_menu_click.ogg"
        ...
        hbox:
            ...
            if not main_menu:
                textbutton _("MAIN MENU"):
                    action MainMenu() 
                    at quick_menu_zoom
                    activate_sound None  # Mutes the click so only pop_up.ogg plays!
            
            if renpy.variant("pc"):
                textbutton _("QUIT"):
                    action Quit(confirm=True) 
                    at quick_menu_zoom
                    activate_sound None  # Mutes the click
...
screen main_menu():
    ...
    on "show" action Play("sound", "audio/ui/keyboard.ogg", loop=True)
    ...
    # 3. The Typing Engine Loop (Fires every 0.02 seconds)
        ...
        If(not is_done,
            If(curr_char < len(boot_lines[curr_line][0]),
                ...
                If(curr_line < len(boot_lines) - 1,
                    ...
                    # MODIFY THIS LINE: Stop the sound when it finishes typing!
                    [SetScreenVariable("is_done", True), Stop("sound")] 
                )
            )
        )
    ]
        ...
        vbox:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    # 1. Wrap the slot in a 'fixed' box so we can overlay the delete button
                    fixed:
                        xsize 380
                        ysize 310

                        # 2. The Main Save/Load Button (Your original code!)
                        button:
                        ...
                        # 3. The Delete Button Overlay
                        # (FileLoadable checks if a save actually exists here. If it's empty, no 'X' appears!)
                        if FileLoadable(slot):
                            textbutton "X":
                                # This triggers your custom Confirm pop-up automatically!
                                action FileDelete(slot, confirm=True)
                                style "slot_delete_button"
                                
                                # Audio!
                                hover_sound "audio/ui/hover.ogg"
                                activate_sound "audio/ui/exit.ogg"
...
style slot_delete_button:
    # Pinned tightly to the top right corner
    align (1.0, 0.0) 
    offset (-6, 6) 
    padding (12, 4)
    
    # IDLE: A subtle dark box so the red text stands out over any screenshot
    idle_background Solid("#050505cc") 
    
    # HOVER: The box itself turns into a solid, bright crimson red block!
    hover_background Solid("#ff4d4d") 

style slot_delete_button_text:
    font gui.name_text_font # Orbitron
    size 22
    
    # IDLE: The 'X' text is naturally red
    idle_color "#ff4d4d" 
    
    # HOVER: The 'X' turns pure white to pop against the solid red hover box
    hover_color "#ffffff"
...
style slot_button:
    ...
    hover_sound "audio/ui/hover.ogg"
    activate_sound "audio/ui/confirm.ogg"
...
screen confirm(message, yes_action, no_action):
    ...
    # PLAY POP-UP SOUND WHEN THIS SCREEN APPEARS
    on "show" action Play("sound", "audio/ui/pop_up.ogg")
    ...
    frame:
        ...
        vbox:
            ...
            hbox:
                ...
                # OVERRIDE the YES button to play confirm.ogg
                textbutton _("YES"):
                    action yes_action 
                    style "confirm_button"
                    activate_sound "audio/ui/confirm.ogg"
                
                # NO gets an explicit override to play exit.ogg
                textbutton _("NO"):
                    action no_action 
                    style "confirm_button"
                    activate_sound "audio/ui/exit.ogg"
...
screen shock_timer():
    # After 5.0 seconds, it fades out the sound and hides this invisible screen
    timer 5.0 action [Stop("sound", fadeout=1.0), Hide("shock_timer")]